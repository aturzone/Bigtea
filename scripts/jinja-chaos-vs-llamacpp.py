"""Compare **Chaos's** chat framing against llama.cpp's, both paths, per container.

`jinja-vs-llamacpp.py` compares llama.cpp's hardcoded renderer against
llama.cpp's own Jinja — it never runs Chaos, despite what its docstring said.
...
script does.
    ...
    python scripts/jinja-chaos-vs-llamacpp.py [model.gguf ...]
...
# chaos: `prompt     6 tokens: [1, 450, 7483, 310, 3444, 338]`
...
def chaos_tokens(model: Path, jinja: bool) -> list[int] | None:
    cmd = [
        str(CHAOS), "-m", str(model), "-sys", SYSTEM, "-p", USER,
        "-n", "1", "--temp", "0", "--force", "-cnv", "--verbose-prompt",
    ]
    if jinja:
        cmd.append("--jinja")
    m = CHAOS_IDS.search(run(cmd))
...
def main() -> int:
    ...
    # apply. Chaos's `-cnv` applies its neutral framing. Reporting that as a
    ...
    for m in models:
        ...
        for label, jinja in (("jinja  ", True), ("family ", False)):
            ours, theirs = chaos_tokens(m, jinja), llama_tokens(m, jinja)
