...
- **Routing is not bitwise stable across sequence lengths.** At 63 → 64 tokens the *same* earlier tokens re-routed ~3% of their selections (net still +6 per layer, so nothing was lost) — the router's top-6-of-256 flipping on near ties when ggml changes matmul blocking at a size boundary. Every other length was exact. Anything assuming reproducible routing across batch shapes — a prefetcher, a replay, R3's equivalence test — must tolerate it, and a test demanding equality will fail on correct code.
