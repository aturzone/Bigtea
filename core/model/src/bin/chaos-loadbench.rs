...
//!
//! Usage: `chaos-loadbench <any-shard.gguf> [--budget GIB]`
//!
...
fn main() -> ExitCode {
    ...
    // `chaos-loadbench --version` reported "cannot find the file specified" -- the
    ...
    if std::env::args()
        ...
        .any(|a| a == "--version" || a == "-V")
    {
        println!("chaos-loadbench {}", env!("CARGO_PKG_VERSION"));
        std::process::exit(0);
    }
    let mut args = std::env::args().skip(1);
    let Some(path) = args.next() else {
        eprintln!("usage: chaos-loadbench <any-shard.gguf> [--budget GIB]");
