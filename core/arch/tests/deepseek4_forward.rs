...
use chaos_arch::{AttentionKind, Deepseek4Config, Deepseek4Model};
...
fn layers_compose_through_the_first_compressed_layer() {
    ...
    assert_eq!(
        ...
        Some(chaos_arch::AttentionKind::Raw)
    );
    ...
    assert_eq!(
        ...
        Some(chaos_arch::AttentionKind::CompressedSparse),
...
fn every_layer_runs_at_two_tokens() {
    ...
    let tok = chaos_tokenizer::Tokenizer::from_metadata(model.metadata()).ok();
    let text = tok
        .as_ref()
...
fn attention_5tok<'c>(
    ...
    let mask = ctx
        .new_typed_2d(chaos_gguf::GgmlType(1), n_kv, nt)
        .expect("mask");
...
/// `chaos_arch::prefill` — the shipped path, the one `chaos-run` will call —
...
fn the_library_forward_pass_matches_llama_cpp() {
    ...
    let logits = chaos_arch::prefill(&fw, TOKENS_2, 1024 << 20).expect("prefill");
    assert_eq!(
        logits.len(),
        config.vocab_size as usize,
        "one logit per token id"
...
fn repacked_resident_weights_match_llama_cpp() {
    ...
    let (mut resident, report) =
        chaos_model::ResidentSet::load(&model, BUDGET).expect("load resident");
    let repacked =
        chaos_arch::RepackedDense::build(&mut resident, &model).expect("repack resident");
...
fn repacking_hands_bytes_over_rather_than_duplicating_them() {
    ...
    const BUDGET: u64 = 3 << 30;
    let (mut resident, _) = chaos_model::ResidentSet::load(&model, BUDGET).expect("load resident");
...
fn the_expert_cache_does_not_change_the_answer() {
    ...
    let fw = chaos_arch::Deepseek4Forward::new(&model, config.clone()).with_expert_cache(4 << 30);

    let cold = chaos_arch::prefill(&fw, TOKENS_2, 1024 << 20).expect("cold prefill");
    ...
    let warm = chaos_arch::prefill(&fw, TOKENS_2, 1024 << 20).expect("warm prefill");
...
fn a_single_pass_larger_than_the_ring_is_refused_not_a_panic() {
    ...
    let fw = chaos_arch::Deepseek4Forward::new(&model, config);

    let too_long: Vec<i32> = (0..1200).map(|i| i % 1000).collect();
    match chaos_arch::prefill(&fw, &too_long, 1024 << 20) {
        Err(chaos_arch::ArchError::ContextTooLong { tokens, limit }) => {
...
fn past_the_old_256_cap_a_cached_step_agrees_with_a_full_prefill() {
    ...
    let full = chaos_arch::prefill(&fw, &tokens, arena).expect("full prefill past 256");

    let mut cache = chaos_arch::Deepseek4Cache::new(config.n_layer, config.kv_lora_rank);
    let _ = chaos_arch::forward(&fw, &mut cache, &tokens[..n], arena).expect("partial prefill");
    assert_eq!(cache.n_past(), n);
    let stepwise = chaos_arch::step(&fw, &mut cache, tokens[n], arena).expect("cached step");
...
fn a_cached_step_agrees_with_a_full_prefill() {
    ...
    let full = chaos_arch::prefill(&fw, &tokens[..=n], arena).expect("full prefill");
...
fn a_cached_step_agrees_through_the_compressed_path() {
    ...
    let mut cache = chaos_arch::Deepseek4Cache::new(config.n_layer, config.kv_lora_rank);
    chaos_arch::forward(&fw, &mut cache, &tokens[..n], arena).expect("partial prefill");
    let stepwise = chaos_arch::step(&fw, &mut cache, tokens[n], arena).expect("cached step");
...
fn cached_step_routing_versus_full_prefill_routing() {
    ...
    let fw = chaos_arch::Deepseek4Forward::new(&model, config.clone());
    let arena = 1024 << 20;
    std::env::set_var("CHAOS_ROUTING_LAST", "1");

    let tokens: Vec<i32> = TOKENS_5.to_vec();
    let n = 2usize;

    chaos_arch::routing_last_token_reset();
    chaos_arch::prefill(&fw, &tokens[..=n], arena).expect("full prefill");
    let full = chaos_arch::routing_last_token();

    chaos_arch::routing_last_token_reset();
    let mut cache = chaos_arch::Deepseek4Cache::new(config.n_layer, config.kv_lora_rank);
    chaos_arch::forward(&fw, &mut cache, &tokens[..n], arena).expect("partial");
    chaos_arch::step(&fw, &mut cache, tokens[n], arena).expect("step");
    let stepwise = chaos_arch::routing_last_token();
