...
//!
//! ```text
//! chaos-gpubench <model.gguf> [--device N] [--limit-gib X]
//! ```
//!
...
use chaos_arch::{KvCache, Qwen3Config, StreamingRunner};
use chaos_ggml::{devices, Context, DeviceKind, Residency, WeightSet};
...
fn prefill_series(
    ...
    for r in 0..=repeats {
        ...
        if device.is_some() && r == 1 {
            ...
            chaos_ggml::backend::timing::reset();
        }
...
fn main() -> ExitCode {
    ...
    // `chaos-gpubench --version` reported "cannot find the file specified" -- the
    ...
    if std::env::args()
        ...
        .any(|a| a == "--version" || a == "-V")
    {
        println!("chaos-gpubench {}", env!("CARGO_PKG_VERSION"));
        std::process::exit(0);
    }
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("chaos-gpubench: {e}");
            ExitCode::FAILURE
        }
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let path = args.next().ok_or(
        "usage: chaos-gpubench <model.gguf> [--device N] [--limit-gib X]\n\
         \n\
...
\n         narrower than the kernels' own spread.",
    ...
    let backend = chaos_ggml::Backend::open(index)?;
    ...
    let n_prompt: usize = std::env::var("CHAOS_PREFILL_TOKENS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(PREFILL_TOKENS);
    let prompt: Vec<u32> = match text_prompt.as_deref() {
        Some(text) => {
            let tokenizer = chaos_tokenizer::Tokenizer::from_metadata(model.metadata())?;
...
prefill {} tokens, same prompt, same session",
    ...
    let (realize_s, up_s, down_s, comp_s, realize_n, comp_n) =
        chaos_ggml::backend::timing::snapshot();
