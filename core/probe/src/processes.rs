...
//!
//! This is legitimate here in a way it never would be on a server: Chaos runs
...
mod tests {
    ...
    fn our_own_toolchain_is_protected() {
        ...
        for name in ["chaos.exe", "cargo.exe", "rustc"] {
            assert!(is_protected(name), "{name} must be protected");
        }
    }
    ...
    fn enumeration_finds_processes_and_never_includes_us() {
        let procs = list();
        // Every platform Chaos claims to support must be able to answer this,
