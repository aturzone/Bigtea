...
//! templates are matched by family" while `crates/chaos-jinja` was evaluating
...
//! drift the same way. The table is extracted from `chaos-run.rs` at test time,
...
fn refused_flags() -> Vec<String> {
    // `src/main.rs`, not `src/bin/chaos-run.rs`: this binary is its own crate
    // in `cli/` now rather than one of several inside `chaos-arch`.
...
fn declined_flags_actually_decline() {
    ...
    let bin = env!("CARGO_BIN_EXE_chaos-run");
    ...
    for flag in &flags {
        ...
        // — `chaos-run --jinja ignored` exits 2 with "no model given", the same
...
fn an_unknown_flag_is_an_error_rather_than_the_prompt() {
    ...
    // with it. `chaos-run -m m -fa off "hello"` set prompt = "-fa", exited 0,
...
fn help_and_version_are_recognised_anywhere_not_only_first() {
    // These were `args().nth(1)` comparisons, so `chaos-run -m model.gguf
    ...
    assert!(stdout.contains("chaos-run"), "stdout was: {stdout}");
