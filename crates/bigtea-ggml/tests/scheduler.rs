...
use bigtea_ggml::sched::{AlignedBytes, TENSOR_ALIGNMENT};
use bigtea_ggml::{backend, devices, Backend, Context, DeviceKind, HostBuffer, Scheduler};
...
fn misaligned_host_memory_is_refused_rather_than_fatal() {
    ...
    match err {
        Some(bigtea_ggml::GgmlError::Misaligned { required, .. }) => {
