...
use chaos_jinja::{parse, render, Env, Value};
use chaos_tokenizer::chat::Message;
...
fn every_container_template_renders_the_same_both_ways() {
    ...
    for path in containers {
        let name = path
            ...
            .to_string_lossy()
            .to_string();
        let Ok(model) = chaos_model::Model::open_split(path.to_string_lossy().as_ref()) else {
            continue;
        };
        ...
        let prepared = if chaos_jinja::supports_system_role(template) {
            raw
        } else {
            chaos_jinja::merge_system_into_first_user(
                &raw, "
",
            )
        };
