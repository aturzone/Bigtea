...
//! piping `chaos-run` into a file gets a header it has to strip, and a caller
...
//! ```
//!
//! **Status goes to stderr, not stdout.** That is the actual fix: `chaos-run
    ...
    let file = config.file.as_ref().and_then(|path| {
            ...
            .open(path)
        {
            ...
            Err(e) => {
                ...
                eprintln!("chaos: cannot open log file {path}: {e}");
                None
            }
        }
    });
