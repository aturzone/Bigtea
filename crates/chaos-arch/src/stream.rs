...
use chaos_ggml::{arena_for, Context, RopeParams, Tensor, WeightSet};
...
pub struct StreamingRunner<'m> {
    ...
    /// on every position. Empty unless `CHAOS_EXPERT_WORKERS` is set.
    ...
    device: Option<chaos_ggml::Backend>,
    ...
    cpu_backend: Option<chaos_ggml::Backend>,
...
///
/// `-t` sets `CHAOS_THREADS` before any model is opened. **It used to be read
...
/// spelled variable (`CHAOS_EXPERT_THREADS`) was read here into a binding
...
/// [`chaos_probe::saturating_threads`].
...
pub fn configured_threads() -> usize {
    env_threads("CHAOS_THREADS").unwrap_or_else(|| logical_cores().clamp(1, 4))
}
...
pub fn configured_threads_batch() -> usize {
    env_threads("CHAOS_THREADS_BATCH").unwrap_or_else(logical_cores)
}
...
impl<'m> StreamingRunner<'m> {
    pub fn new(model: &'m Model, config: Qwen3Config, cache_budget: usize) -> Self {
        ...
        StreamingRunner {
            ...
            tuner: if env_threads("CHAOS_THREADS").is_some() {
                None
            } else {
    ...
    pub fn use_device(&mut self, index: usize) -> Result<()> {
        self.device = Some(chaos_ggml::Backend::open(index)?);
        Ok(())
    }
    ...
    pub fn set_op_offload(&mut self, on: bool) -> Result<()> {
        if on && self.cpu_backend.is_none() {
            self.cpu_backend = Some(chaos_ggml::Backend::cpu()?);
        }
        self.op_offload = on;
        Ok(())
    }
    ...
    fn target<'x>(
        sched: Option<&'x chaos_ggml::Scheduler<'x>>,
        device: Option<&'x chaos_ggml::Backend>,
        threads: usize,
    ) -> chaos_ggml::Compute<'x> {
        match sched {
            Some(s) => chaos_ggml::Compute::Sched(s),
    ...
    fn layer_device_at<'b>(
        device: Option<&'b chaos_ggml::Backend>,
        placement: &[bool],
        gpu_layers: usize,
        il: usize,
    ) -> Option<&'b chaos_ggml::Backend> {
    ...
    fn compute_with(
        device: Option<&chaos_ggml::Backend>,
        threads: usize,
    ) -> chaos_ggml::Compute<'_> {
        match device {
            Some(b) => chaos_ggml::Compute::Device(b),
            None => chaos_ggml::Compute::Cpu { threads },
        }
    }
    ...
    /// left to split. `CHAOS_EXPERT_WORKERS` overrides it on hardware this was
    ...
    fn expert_workers(&self, n_experts: usize) -> usize {
        ...
        let want = std::env::var("CHAOS_EXPERT_WORKERS")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
    ...
    fn tensor_type(&self, name: &str) -> Result<chaos_gguf::GgmlType> {
        self.model
            .location(name)
            .map(|l| l.ty)
            .ok_or_else(|| ArchError::MissingTensor(name.to_string()))
    ...
    pub fn load_resident_on_device<'a>(
        ...
        u64,
        Option<chaos_ggml::DeviceBuffer>,
        chaos_ggml::UploadReport,
    )> {
    ...
    pub fn load_resident_split<'a>(
        ...
        let repack = std::env::var("CHAOS_NO_REPACK").is_err() && !self.op_offload;
    ...
    pub fn load_resident<'a>(
        ...
        //
        // `CHAOS_NO_REPACK` turns it off, for measuring the difference or for
        // a machine where the rearranged copy is unaffordable.
        let repack = std::env::var("CHAOS_NO_REPACK").is_err();
    ...
    fn forward_scheduled<'a>(
        ...
        match (self.op_offload, device.as_ref(), cpu.as_ref()) {
            (true, Some(gpu), Some(host)) => {
                ...
                let sched = chaos_ggml::Scheduler::new(&backends, 8192, true)?;
    ...
    fn forward_cached_inner<'a>(
        ...
        pos_start: usize,
        device: Option<&chaos_ggml::Backend>,
        sched: Option<&chaos_ggml::Scheduler<'_>>,
    ) -> Result<Vec<f32>> {
        let c = self.arch.config.clone();
        ...
        let trace = std::env::var("CHAOS_GPU_TRACE").is_ok();
