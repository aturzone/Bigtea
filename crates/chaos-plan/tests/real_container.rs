...
//! machine that has not downloaded 144 GiB. Point `CHAOS_TEST_GGUF` at a
//! container to run it.
...
use chaos_gguf::Gguf;
use chaos_plan::{ModelProfile, Prediction, ProfileSource, DEFAULT_EFFICIENCY, GIB};
...
fn parses_a_real_deepseek_container() {
    let Some(path) = find_container() else {
        eprintln!("skipping: no container found (set CHAOS_TEST_GGUF)");
        return;
    };
...
fn every_tensor_in_a_real_container_can_be_sized() {
    ...
    // model's real cost, which is the one number Chaos exists to get right.
    let Some(path) = find_container() else {
