...
//! That is not hypothetical; it is why `chaos-pull` passes `--fail`, and a 401
...
///
/// `CHAOS_CACHE` wins, then the platform cache directory. Not the current
...
pub fn cache_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("CHAOS_CACHE") {
...
pub fn fetch(
    ...
    if Command::new("curl").arg("--version").output().is_err() {
        return Err(
            "curl was not found on PATH, and it is how Chaos downloads. Install curl, \
...
mod tests {
    ...
    fn a_file_that_is_not_gguf_is_not_mistaken_for_one() {
        let dir = std::env::temp_dir().join("chaos-dl-test");
