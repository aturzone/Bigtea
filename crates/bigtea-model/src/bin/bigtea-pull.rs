...
//!
//! Usage: `bigtea-pull <model> [--quant NAME] [--dir PATH] [--yes] [--dry-run]`
//!
...
use bigtea_model::catalogue::{self, gib, Plan};
...
fn usage() {
    println!("usage: bigtea-pull <model> [--quant NAME] [--dir PATH] [--yes] [--dry-run]");
    println!();
    println!("  --list      what Bigtea can fetch");
...
fn run(
    ...
    let Some(entry) = catalogue::find(model) else {
        eprintln!("bigtea-pull: no model called {model:?}. Known models:");
        list();
    ...
    let machine = bigtea_probe::Machine::probe(dir, false);
    let plan = Plan {
        entry,
        quant,
        files: files.clone(),
...
/// Fetch through `curl`.
///
/// Bigtea has **no external Rust dependencies** — the whole workspace is path
...
fn fetch(
    ...
    if Command::new("curl").arg("--version").output().is_err() {
        return Err(
            "curl was not found on PATH, and it is how Bigtea downloads. \
