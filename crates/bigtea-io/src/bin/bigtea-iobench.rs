...
//!
//! Usage: `bigtea-iobench <big-file> [--slice-mib N] [--slices N]`
//!
//! # The suspicion
//!
...
use bigtea_io::{DirectFile, SkewedBuf};
...
fn main() -> ExitCode {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-h" | "--help" => {
                println!("usage: bigtea-iobench <big-file> [--slice-mib N] [--slices N]");
                println!();
    ...
    let Some(path) = path else {
        eprintln!("usage: bigtea-iobench <big-file> [--slice-mib N] [--slices N]");
