...
//!
//! Usage: `bigtea-spectrum <model.gguf> [--layer N] [--tensor up|gate|down]
...
use bigtea_arch::spectrum::{energy_curve, gaussian, Gram};
use bigtea_ggml::Context;
...
fn usage() {
    println!("usage: bigtea-spectrum <model.gguf> [options]");
    println!();
...
fn accumulate(
    ...
    for e in 0..sampled {
        let w: Vec<f32> = if random {
            ...
            bigtea_ggml::dequantize(loc.ty, &raw, elements)?
        };
