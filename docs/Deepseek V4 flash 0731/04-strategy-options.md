# 04 — Strategy options for Chaos (post-WASTE)

Context in one line: Chaos wraps engines (llama.cpp, ktransformers, SGLang) and never forks
...
- **Effort vs value:** highest value/effort of anything Chaos can do this quarter; it turns a
...
- **Why now:** WASTE, ds4, llama.cpp, FATE each solve *one* engine. **Nobody** offers a
  ...
  benchmark schema keyed to working-set multiples. Chaos's planned CLI (probe/recommend/launch/
  bench, wrapper-core) is already this shape.
- **The move:** benchmark-harness T2 + T6 and gap-closure T2 become the "why choose Chaos"
...
- **Why:** both are Apache-2.0/MIT-adjacent, CLI/API-friendly, and already do the disk-streaming
  lane Chaos would never build. Wrapping them later costs nothing now; the ADR's dynamic-tracking
...
- **The move:** add a line to the future-backend research queue (in `05-actions.md`), with a
  ...
  matrix Chaos can actually target.

## What NOT to chase (the "don't" list)

- **The frontier-Mac lane.** ds4 owns the "128 GB Mac streams the biggest model" story and has
  13k stars; WASTE owns the K3-on-64GB story. Chaos's mvp is Linux+NVIDIA. Do not reposition to
...
- **Chasing star-count.** Traction here is narrative; Chaos's winnable game is *expertise* in
...
levers so Chaos never rebuilds them (Option C). Treat WASTE not as a rival to out-engine but as
a calibrated, public oracle for the exact problem Chaos is best positioned to *operationalize*.
