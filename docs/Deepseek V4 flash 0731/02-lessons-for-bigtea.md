# 02 — Lessons for Bigtea (map to tickets)

Every lesson below is drawn from `01-waste-deep-dive.md` and ends at a concrete Bigtea ticket or
...
engine, and it is precisely the effect Bigtea's benchmark harness was designed to observe.
...
- `backlog/gap-closure.md` **T2** (split/offload auto-recommender): adopt the deterministic
  ...
  heuristic that Bigtea's research node flags as buggy** — a working-set-multiple rule is the
...
WASTE's measured refutations are a buy-list for Bigtea's *research* node on offload
...
with read-ahead. Bigtea's `gap-closure` T4/T5 observability work models "decode ceiling" as a
...
project is trusted within a week. Bigtea's graph already has the right instincts (recorder gate,
...
real floor was 30.38 GB, not the planned 29.69. This is a direct endorsement of Bigtea's
...
of Bigtea's offload pitch to users. The MoE expert cache is the centerpiece; KV is already
