...
Author: Deepseek V4 flash (0731) — written as an advisory from me to Atur, for the Bigtea project.
...
llama.cpp MoE hot-cache, FATE/llama-moe-cache, ktransformers) and a full read of Bigtea's
knowledge graph and current code (`bigtea/*.py`, `tests/*.py`).
...
WASTE is not a direct competitor to Bigtea — it is an inference *engine* (it *is* the backend,
like llama.cpp or ktransformers), while Bigtea is a *wrapper* over such engines. The real threat
...
Bigtea planned to make its own (gap-closure T4/T5 observability, benchmark-harness T6
...
Bigtea's T4/T5/T6 for the llama.cpp side. The good news: **three independent engines converged on
the same physical model** (working-set floor, pressure-zone ceiling, ~80% RAM-cap), which
*validates* Bigtea's measurement-methodology bets; and the **wrapper lane is still unclaimed** —
...
| `02-lessons-for-bigtea.md` | Each transferable lesson mapped to a concrete Bigtea ticket (gap-closure, benchmark-harness, hardware-profiler, wrapper-core). |
| `03-landscape-2026.md` | The competitive field: WASTE vs ds4 vs llama.cpp hot-cache vs FATE vs ktransformers; the convergence finding; the staleness warning on Bigtea's gaps research. |
| `04-strategy-options.md` | Where Bigtea stays differentiated; what not to chase; risk register. |
| `05-actions.md` | Proposed (not applied) graph/backlog actions, with acceptance hints. |

## The one-sentence takeaway

> Bigtea should treat WASTE (and ds4) as *evidence that the cache-hit-rate and offload-tuning
> problem is real and worth solving*, as *input that sharpens Bigtea's benchmark and split
> methodology*, and as *candidate future backends to wrap* — not as something to out-engine.
> The engine-writing lane is a different game, and Bigtea's ADR already chose not to play it.

## Links into the graph (for cross-referencing)

- `docs/graph/research/benchmarking-methodology.md` — Bigtea's planned metric + protocol; lesson 2–3 land here.
