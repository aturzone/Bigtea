...
- research/mvp-scope.md — realistic 4-6wk v1 for a 1-2 dev team: T0 gates week 1 with a fail-path that drops observability entirely; v1 cut = wrapper core (new: CLI, version-detection, launch-flag glue) + gap-closure T0/T1/T2 + hardware-profiler T1/T2/T4 + benchmark-harness T1/T2, targeting one model class (DeepSeek-class MoE) on one platform (Linux+NVIDIA); sanity-checked against ollama/ramalama's equally narrow v1s — links: ../decisions/fork-vs-wrapper.md, hardware-profiling.md, benchmarking-methodology.md, backlog/wrapper-core.md, backlog/mvp-v1.md
...
- backlog/mvp-v1.md — v1 milestone plan (no new tickets): week-by-week sequencing over existing gap-closure/hardware-profiler/benchmark-harness/wrapper-core tickets, incl. T0-fails fallback path — links: ../research/mvp-scope.md, ../decisions/fork-vs-wrapper.md, wrapper-core.md
