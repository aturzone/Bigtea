...
**2. Biases beyond Q/K/V.** #60 added `attn_bias`, detected from
`blk.0.attn_q.bias`, covering **Q/K/V only** — confirmed with the r11 session.
`attn_output.bias`, `ffn_up.bias` and `ffn_down.bias` do not exist, and
StarCoder2 needs all three:
...
5. Non-gated FFN with GELU when `ffn_gate` is absent — an ungated variant of
   `FfnAct`, which #60 introduced.

**Verify with `scripts/parity-check.sh`, not a single prompt.** Three prompts
(factual, narrative, code) x 32 tokens x `--temp 0`, both engines. "The capital
of France is Paris" survives a surprising amount of wrong arithmetic; the code
prompt is what exposes things. The script also re-runs the reference under a
second configuration before calling anything a failure, because **llama.cpp
disagrees with itself on near-tie prompts** — `def fibonacci(n):` on Llama-3.2-1B
changes under `-fa`, and Phi-3 changes under `--no-repack`. A mismatch is not
automatically a bug.
